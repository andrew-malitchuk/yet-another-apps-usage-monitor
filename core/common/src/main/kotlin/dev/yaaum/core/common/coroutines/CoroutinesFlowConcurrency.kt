package dev.yaaum.core.common.coroutines
...
internal const val CHANNEL_DEFAULT_CAPACITY = 64
 ...
 * `transform(a)` or `transform(c)` complete, `transform(c)` will be cancelled but `transform(a)`
 * will be allowed to complete.
 */
@Suppress("LoopWithTooManyJumpStatements", "TooGenericExceptionCaught")
fun <T, R> Flow<T>.flatMapConcurrently(
    ...
    return flow {
        ...
        supervisorScope {
            ...
            while (true) {
                ...
                exceptionWasThrownEarlier.invokeOnCompletion { thrown -> // should never be null
                    ...
                    // it's possible that evalTransform completed successfully, but there are
                    // other downstream transform's to cancel, so we can't depend on
                    // the evalTransform completion handler to propagate thrown
                    exceptionWasThrownEarlierOrHere.completeExceptionally(thrown)
                }
                emit(evalTransform)
        ...
        //
        // "It is easy to remember. Without buffer there is no concurrency - at most 1 operationa
        // is executed at any time, with buffer(0) at most 2 can run concurrently,
        // so with buffer(n) at most n + 2."
