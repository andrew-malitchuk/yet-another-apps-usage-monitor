...
@Composable
fun YaaumPullToRefreshLayout(
    modifier: Modifier = Modifier,
    pullRefreshLayoutState: YaaumPullToRefreshLayoutState,
    onRefresh: () -> Unit,
    ...
    val pullToRefreshState = rememberPullRefreshState(
        refreshing = refreshIndicatorState == YaaumRefreshIndicatorState.Refreshing,
    ...
    LaunchedEffect(key1 = pullToRefreshState.progress) {
        @Suppress("OptionalWhenBraces", "OptionalWhenBraces")
        when {
            pullToRefreshState.progress >= 1 -> {
                pullRefreshLayoutState.updateRefreshState(YaaumRefreshIndicatorState.ReachedThreshold)
            }

            pullToRefreshState.progress > 0 -> {
                pullRefreshLayoutState.updateRefreshState(YaaumRefreshIndicatorState.PullingDown)
            }
        }
    }
